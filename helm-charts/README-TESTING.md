...
```bash
cd /Users/tviviano/Documents/GitHub/trv-edgelake-infra/helm-charts
...
**Master Node:** trv-srv-001 (100.127.19.27:32048)
...
# View logs
ssh tviviano@100.74.102.38 'kubectl logs -f -l app=edgelake-k8s-test'
```
...
**View logs:**
```bash
ssh tviviano@100.74.102.38 'kubectl logs -f -l app=edgelake-k8s-test -n default'
```

**Shell access:**
```bash
ssh tviviano@100.74.102.38 'kubectl exec -it $(kubectl get pod -l app=edgelake-k8s-test -n default -o jsonpath="{.items[0].metadata.name}") -- /bin/bash'
```

**EdgeLake CLI access:**
```bash
ssh tviviano@100.74.102.38 'kubectl exec -it $(kubectl get pod -l app=edgelake-k8s-test -n default -o jsonpath="{.items[0].metadata.name}") -- edgelake-cli'
```

**Pod details:**
```bash
ssh tviviano@100.74.102.38 'kubectl describe pod -l app=edgelake-k8s-test -n default'
```
...
**Pod not starting:**
```bash
ssh tviviano@100.74.102.38 'kubectl describe pod -l app=edgelake-k8s-test'
ssh tviviano@100.74.102.38 'kubectl logs -l app=edgelake-k8s-test --previous'
```

**PVC issues:**
```bash
ssh tviviano@100.74.102.38 'kubectl get pvc -l app=edgelake-k8s-test'
ssh tviviano@100.74.102.38 'kubectl describe pvc -l app=edgelake-k8s-test'
```

**Network issues:**
```bash
# Test from pod to master
ssh tviviano@100.74.102.38 'kubectl exec $(kubectl get pod -l app=edgelake-k8s-test -o jsonpath="{.items[0].metadata.name}") -- nc -zv 100.127.19.27 32048'
...
**Service issues:**
```bash
ssh tviviano@100.74.102.38 'kubectl get svc -l app=edgelake-k8s-test -o wide'
ssh tviviano@100.74.102.38 'kubectl get endpoints -l app=edgelake-k8s-test'
```
...
# Verify removal
ssh tviviano@100.74.102.38 'kubectl get all -l app=edgelake-k8s-test'
```
...
```bash
# From k8s host
ssh tviviano@100.74.102.38 'nc -zv 100.127.19.27 32048'
...
**Fix:**
```bash
ssh tviviano@100.74.102.38 'kubectl describe pod -l app=edgelake-k8s-test | grep -A 5 Events'
```
