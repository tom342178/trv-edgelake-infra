...
### OH Hub Server (tviviano@100.127.19.27)
**Components Running:**
...
### Existing Edge Node (tviviano@100.102.221.116)
...
wget http://100.127.19.27/agent-install.sh
...
```bash
cd /Users/tviviano/Documents/GitHub/demo-in-a-box-edgelake
...
export DOCKER_IMAGE_BASE=100.127.19.27:5000/edgelake-mcp  # Your registry
...
LEDGER_CONN=100.127.19.27:32048  # Your master node
...
sed -i 's|anylogco/edgelake|100.127.19.27:5000/edgelake-mcp|g' edgelake/service.definition.json
```
...
```json
{
  "deployment": {
    "services": {
      "$SERVICE_NAME": {
        "image": "100.127.19.27:5000/edgelake-mcp:amd64-latest",
        "network": "host",
        "privileged": true,
        "binds": [
...
On the new device (e.g., new device or re-register 100.102.221.116):
...
```bash
# Get container name
CONTAINER_NAME=$(docker ps --filter "ancestor=100.127.19.27:5000/edgelake-mcp:amd64-latest" --format "{{.Names}}")
...
curl http://100.127.19.27:32049/status
...
ssh tviviano@100.127.19.27 "hzn agbot agreement list"
```
...
docker pull 100.127.19.27:5000/edgelake-mcp:amd64-latest
...
telnet 100.127.19.27 32048
telnet 100.127.19.27 32049
...
3. **Deployment Approach Options:**
   ...
   **Option B: Hybrid Approach (Recommended)**
   ...
   - Leverage your existing helm charts at `/Users/tviviano/Documents/GitHub/utilities/edgelake/helm-charts`
...
helm install agent-operator openhorizon/agent-operator \
  -n openhorizon-agent \
  --set horizon.exchange.url=http://100.127.19.27:3090/v1 \
  --set horizon.css.url=http://100.127.19.27:9443 \
  --set horizon.organization=$HZN_ORG_ID

# Option B: Manual installation
# Download agent operator manifests from your OH hub
wget http://100.127.19.27/agent-operator-install.yaml
...
kubectl create secret generic openhorizon-agent-secrets \
  --from-literal=HZN_EXCHANGE_URL=http://100.127.19.27:3090/v1 \
  --from-literal=HZN_FSS_CSSURL=http://100.127.19.27:9443 \
  ...
  "userInput": [
    {
      ...
      "type": "string",
      "defaultValue": "100.127.19.27:32048"
    },
    {
      "name": "NODE_NAME",
...
```bash
cd /Users/tviviano/Documents/GitHub/utilities/edgelake/helm-charts
...
node_configs:
  ...
  blockchain:
    LEDGER_CONN: "100.127.19.27:32048"
    BLOCKCHAIN_SOURCE: master
    BLOCKCHAIN_SYNC: "30 second"
...
# Check connectivity to master
kubectl exec -n edgelake <operator-pod> -- curl -v 100.127.19.27:32048
```
...
kubectl exec -n openhorizon-agent <agent-pod> -- curl http://100.127.19.27:3090/v1/admin/version
```
...
kubectl exec -n edgelake <operator-pod> -- nc -zv 100.127.19.27 32048
kubectl exec -n edgelake <operator-pod> -- nc -zv 100.127.19.27 32049

# Check service endpoints
kubectl get endpoints -n edgelake

# Test DNS resolution
kubectl exec -n edgelake <operator-pod> -- nslookup 100.127.19.27
```
...
```bash
helm install operator-1 /Users/tviviano/Documents/GitHub/utilities/edgelake/helm-charts/edgelake-operator \
...
2. **Day 3-4:** Deploy to existing operator (100.102.221.116)
...
- **Location:** `/Users/tviviano/Documents/GitHub/demo-in-a-box-edgelake`
...
### Helm Charts
- **Location:** `/Users/tviviano/Documents/GitHub/utilities/edgelake/helm-charts/edgelake-operator`
- **Chart:** `Chart.yaml`
- **Values:** `values.yaml`
- **Templates:** `templates/`
...
- **Location:** `/Users/tviviano/Documents/GitHub/trv-edgelake-infra/trv-docker-compose`
...
3. Do you want to test on 100.102.221.116 first or a new device?
...
- **Location:** `/Users/tviviano/Documents/GitHub/documentation`
...
### Internal Resources
- Docker registry: `100.127.19.27:5000`
- Artifactory: `100.127.19.27:<port>` (to be configured)
- OH Exchange: `http://100.127.19.27:3090/v1`
- OH CSS: `http://100.127.19.27:9443`
