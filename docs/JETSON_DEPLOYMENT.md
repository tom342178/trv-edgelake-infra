...
- Network connectivity to OH Hub (100.127.19.27)
- Tailscale installed and connected (IP: 100.80.4.101)
- SSH access to Jetson

## Architecture

```
OH Hub (100.127.19.27)          Jetson Nano (100.80.4.101)
...
# Build ARM64 image
docker build -t 100.127.19.27:8081/trve-repo-local/edgelake:arm64-latest .

# Login to Artifactory
docker login 100.127.19.27:8081

# Push the image
docker push 100.127.19.27:8081/trve-repo-local/edgelake:arm64-latest
```
...
docker buildx build --platform linux/arm64 \
  -t 100.127.19.27:8081/trve-repo-local/edgelake:arm64-latest \
  --push .
```
...
{
  "insecure-registries": ["100.127.19.27:8081"]
}
EOF
...
curl -sSL http://100.127.19.27:9443/api/v1/objects/IBM/agent_files/agent-install.sh -o agent-install.sh
...
This is done from the OH Hub (100.127.19.27).
...
scp oh-services/operator/node.policy.jetson.json \
  tviviano@100.80.4.101:/home/tviviano/node.policy.json
```
...
hzn register --name=trv-jetson-nano --policy=/home/tviviano/node.policy.json
```
...
curl http://100.80.4.101:32449/get/status
```
...
```bash
curl -s -H "command: blockchain get operator" http://100.127.19.27:32049 | jq .
```
...
1. Check if image can be pulled:
   ```bash
   docker pull 100.127.19.27:8081/trve-repo-local/edgelake:arm64-latest
   ```
...
| Setting | Value |
|---------|-------|
| Jetson Tailscale IP | 100.80.4.101 |
| OH Organization | myorg |
| TCP Port | 32448 |
| REST Port | 32449 |
| Broker Port | 32450 |
| Master (LEDGER_CONN) | 100.127.19.27:32048 |
| Image | 100.127.19.27:8081/trve-repo-local/edgelake:arm64-latest |
| Architecture | arm64 |
